...
# Paths
# NOTE: These paths will be invalid in the Streamlit environment and should be managed
# by your main app.py when running the script within the Streamlit process.
...
def get_logo(team_name: str, logos_folder: str) -> Image.Image:
    ...
    """
    # Clean the team name for matching
    team_name_clean = team_name.strip()
    team_name_lower = team_name_clean.lower()
    
    # 1. Check for SPECIAL MAPPING first
    # This is useful for teams like 'AFC Aldermaston A' mapping to 'AFC Aldermaston.png'
    logo_filename = SPECIAL_LOGO_MAPPING.get(team_name_lower)
    
    # If a mapping exists, search for the MAPPED filename in the subfolders
    if logo_filename:
        for subfolder in ['Current Teams', 'Old Teams']:
            search_path = os.path.join(logos_folder, subfolder, logo_filename)
            if os.path.exists(search_path):
                try:
                    return Image.open(search_path).convert("RGBA").resize((LOGO_WIDTH, LOGO_HEIGHT), Image.Resampling.LANCZOS)
                except Exception as e:
                    print(f"Error loading MAPPED logo '{logo_filename}' for {team_name} from '{search_path}': {e}")

    # 2. Search for the standard team name file (e.g., "Team Name.png") in 'Current Teams' then 'Old Teams'
    for subfolder in ['Current Teams', 'Old Teams']:
        # Construct the path to the file using the clean team name (assuming Team Name.png format)
    ...
    # 3. Fallback to generic logo if not found after checking both folders
...
# --- END OF CLEANED HELPER FUNCTION ---
...
def create_match_graphic_with_heading(sections_to_draw: list[tuple], logos_folder: str, save_folder: str, part_number: int, template_path: str, current_date: datetime):
    ...
    # Calculate Y positions for center alignment
    ...
    # Downscale and paste the circle onto the main image
    ...
    for division_name, matches in sections_to_draw:
        ...
        # Heading Y offset correction: 20px buffer before, 20px after heading text
        ...
        for match in matches:
            ...
            # Draw Cup Name (if applicable and different from the last one)
            if division_name.lower().startswith("cup") and cup_name and cup_name != last_cup_name:
                ...
                # Cup name Y offset correction: 5px buffer before, 10px after cup name text
                ...
                # Add spacing between fixtures within the same group/division
            ...
            # NOTE: Resizing is now handled within get_logo for consistency
            ...
            # Advance Y offset for the next fixture
            y_offset += BOX_HEIGHT
    ...
    # Crop content area to respect SAFE_CONTENT_HEIGHT_LIMIT
    content_img = img.crop((0, int(CONTENT_START_Y), IMAGE_WIDTH, int(CONTENT_START_Y + SAFE_CONTENT_HEIGHT_LIMIT)))
...
def generate_fixtures_graphics(file_path: str, logos_folder: str, save_folder: str, template_path: str):
    ...
    while remaining_cup_divisions:
        ...
        for div_data in remaining_cup_divisions:
            ...
            if division_name.startswith("Cup - Hampshire Trophy Cup"):
                if not trophy_cup_included:
                    # Include all 4 Trophy Cup matches in the first graphic (if it fits)
                    if current_height + division_height <= SAFE_CONTENT_HEIGHT_LIMIT or not sections_to_draw:
                        ...
                        print(f" -> {division_name} doesn't fit. Moving to next cup graphic.")
                # Once included, we skip it on subsequent loops.
                continue
                ...
                if len(matches) > max_matches:
                    ...
                    if current_height + temp_height <= SAFE_CONTENT_HEIGHT_LIMIT or not sections_to_draw:
                        ...
                        next_graphic_divisions.append(div_data)
                        print(f" -> {division_name} ({len(matches)} matches) doesn't fit. Moving to next cup graphic.")
                    continue
                else:
                    # Small enough to fit or full group
                    if current_height + division_height <= SAFE_CONTENT_HEIGHT_LIMIT or not sections_to_draw:
                        sections_to_draw.append(("Cup", matches))
                        current_height += division_height
                        is_first_division = False
        ...
        # Generate the graphic if we collected sections
        if sections_to_draw:
    ...
    while remaining_league_divisions:
        ...
        for div_data in remaining_league_divisions:
            ...
            if current_height + division_height <= SAFE_CONTENT_HEIGHT_LIMIT or not sections_to_draw:
                ...
                next_graphic_divisions.append(div_data)
                print(f" -> {division_name} doesn't fit. Moving to next league graphic.")
