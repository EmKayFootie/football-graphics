...
# Use relative paths for files and folders located within the project's directory.
# This assumes the script, results.xlsx, Logos, Graphics, and Templates folders
# are all within the same top-level directory.

# Paths
# RESULTS_FILE_PATH is now a relative path
...
# IMPORTANT: Font Path - Assuming the font file is also located in the project's root folder
# If the font is a system-wide font and you are running on a server/remote environment, 
# you might need to change this path to a font file you include in your project folder.
...
# Load font only if path exists (safer for environments where it might not be a system font)
try:
    ...
    print(f"Warning: Could not load font from {FONT_PATH} during setup. Using default font metrics.")
...
def get_logo(team_name: str, logos_folder: str) -> Image.Image:
    ...
    # Generate search variants (no spaces)
    ...
    # 1. Check for SPECIAL_LOGO_MAPPING
    ...
    # 2. Check all files in subfolders for best match
    for subfolder in ['Current Teams', 'Old Teams', '']:
        ...
        for filename in os.listdir(current_search_dir):
            ...
            # Check if any search variant is contained within the filename
    ...
    try:
        ...
        # Ensure generic logo path is correct relative to the script's execution
        if os.path.exists(generic_logo_path):
            ...
            print(f"Error: Generic logo not found at {generic_logo_path}. Using gray placeholder.")
            raise FileNotFoundError # Trigger the final fallback
    except Exception as e:
...
def parse_matches_from_file(file_path: str, division: str) -> list[tuple]:
    ...
    matches = []
    try:
        # File is read by the main function, so we just check for existence here
        ...
        for _, row in excel_data.iterrows():
            ...
            # Cup name is in column 5 (index 4)
            cup_name = str(row.iloc[4]).strip() if pd.notna(row.iloc[4]) and division.lower() == "cup" else None
            
            # Penalty score is in column 6 (index 5) - only for Cup division
            penalty_score = None
            if division.lower() == "cup":
                # Ensure the row has at least 6 columns (index 5)
                if len(row) > 5:
                    ...
                    # Optional: Add a warning if Cup division is missing the penalty column
                    # print(f"Note: Cup division found but missing expected Penalty Score column (index 5) for match {team_1_name} vs {team_2_name}.")
                    pass
        ...
        # This error is now more specific, likely an issue with the Excel sheet name/format
        # if the file existence was checked earlier.
...
def wrap_text(text: str, font: ImageFont.FreeTypeFont, max_width: int, draw: ImageDraw.ImageDraw) -> list[str]:
    ...
    try:
        ...
        space_width = font.getbbox(" ")[2] - font.getbbox(" ")[0] if hasattr(font, 'getbbox') else 10
...
def calculate_division_height(division_name: str, matches: list, is_first_division: bool = True) -> int:
    ...
    for j, match in enumerate(matches):
        ...
        # Add cup name space if needed (match[4] is cup_name)
        cup_name = match[4]
...
def create_match_graphic_with_heading(sections_to_draw: list[tuple], logos_folder: str, save_folder: str, part_number: int, template_path: str, current_date: datetime):
    ...
    month_text = current_date.strftime("%B").upper() # Uppercasing month for style consistency with Bebas Neue
    year_text = current_date.strftime("%Y")
    
    # Dynamic font size adjustment for date
    font_size = FONT_SIZE_DATE
    date_font = None # Initialize to avoid scope issues
    total_text_height = DATE_TEXT_MAX_HEIGHT * HIGH_RES_SCALE # Initialize to max
    
    while font_size >= FONT_SIZE_DATE_MIN:
        try:
            date_font = ImageFont.truetype(FONT_PATH, int(font_size * HIGH_RES_SCALE))
        except IOError:
            date_font = ImageFont.load_default() # Use default fallback font
            
        # Get bounding boxes to check fit
        ...
        # Add 5px spacing between lines (scaled)
    ...
    if font_size < FONT_SIZE_DATE_MIN and date_font:
    ...
    # Draw date text (assuming date_font is set)
    if date_font:
    ...
    # Resize back to original size and paste
    ...
    for division_name, matches in sections_to_draw:
        ...
        for match in matches:
            ...
            if not logo_1 or not logo_2:
                ...
                y_offset -= FIXTURE_SPACING # Adjust offset if skipped
                continue
            ...
            # Score Box - Enhanced for penalty display
            ...
            if is_penalty_match:
                ...
                penalties_text_y = regular_score_text_y + regular_score_bbox[3] - regular_score_bbox[1] + 12
                d.text((penalties_text_x, penalties_text_y), penalties_label, fill=(255, 255, 0), font=penalties_label_font)
                ...
                penalty_score_text_y = penalties_text_y + penalties_bbox[3] - penalties_bbox[1] + 8
    ...
    # Final image processing (cropping content area for safety)
    ...
    # Copy the template elements above the content area
    final_img.paste(img.crop((0, 0, IMAGE_WIDTH, int(CONTENT_START_Y))), (0, 0))
    # Copy the content area
    content_img = img.crop((0, int(CONTENT_START_Y), IMAGE_WIDTH, int(CONTENT_START_Y + max_content_height)))
    final_img.paste(content_img, (0, int(CONTENT_START_Y)))
    # Copy the template elements below the content area (footer)
    final_img.paste(img.crop((0, int(CONTENT_START_Y + max_content_height), IMAGE_WIDTH, IMAGE_HEIGHT)), (0, int(CONTENT_START_Y + max_content_height)))
...
def generate_results_graphics(file_path: str, logos_folder: str, save_folder: str, template_path: str):
    ...
    if not os.path.exists(file_path):
        print(f"CRITICAL ERROR: File not found at '{file_path}'. Please ensure the file is in the script's execution directory.")
        # If running in a Streamlit environment, this path refers to the CWD of the Streamlit process.
        # If your 'fixtures' code works, this path should resolve correctly.
        return
    ...
    try:
        ...
        if not date_df.empty and 'Date' in date_df.columns:
            ...
            if pd.notna(parsed_date):
                ...
                print(f"Warning: Could not parse date format: {date_str}. Using current date.")
                
        else:
            print(f"Warning: 'Date' sheet empty or missing 'Date' column. Using current date.")
    ...
    # CONTENT_HEIGHT_LIMIT is 1050. SAFE_CONTENT_HEIGHT_LIMIT is the practical maximum.
    SAFE_CONTENT_HEIGHT_LIMIT = 960 
    ...
    while remaining_divisions:
        ...
        for div_data in remaining_divisions:
            ...
            # --- Special/Conservative Fit Check (as per original logic) ---
            ...
            # Always allow the first division to be added, even if it exceeds the safe limit,
            # but must not exceed the absolute limit (CONTENT_HEIGHT_LIMIT).
            if is_first and total_height > CONTENT_HEIGHT_LIMIT:
                 # This single division is too large for the content area.
                 print(f" -> CRITICAL: {division_name} ({division_height}px) is too tall to fit on a single graphic ({CONTENT_HEIGHT_LIMIT}px). Skipping/Error.")
                 next_graphic_divisions.append(div_data)
                 continue 
        ...
        if not sections_to_draw and remaining_divisions:
            print("Error: Remaining divisions are too large to fit on a single graphic, even the first one.")
            break
